...
Precomputing these constants reduced my computation time by two-thirds for a simple scene (1x plane, 2x spheres, 2x cylinders). This improvement is especially noticeable when using memory-checking tools such as Valgrind, reducing the compilation time from  ~35 sec to ~12 sec.
